...
class HttpDecisionClient:
    def __init__(self, url="http://127.0.0.1:8000", timeout=600):
        self.url = url.rstrip("/") + "/v1/calibrated-schema"
    ...
    def decide(self, state, schema, image):
        ...
        self.model_id = result.get("model")
        return result
...
class LitJevPolicy:
    def __init__(self, client, action_names, instructions):
        if not 2 <= len(action_names) <= 26 or len(set(action_names)) != len(action_names):
            raise ValueError("Visual policy requires 2–26 unique action names")
        self.client = client
        self.action_names = tuple(action_names)
        self.instructions = instructions
        self.labels = tuple(string.ascii_uppercase[: len(action_names)])
        self.schema = {
            "action": {
                "type": "enum",
                "choices": list(self.labels),
                "description": "Which controller button should be pressed next?\n"
                + "\n".join(
                    f"{label}. {name}"
                    for label, name in zip(self.labels, self.action_names, strict=True)
                ),
            }
        }
    ...
    def decide(self, observation):
        ...
        result = self.client.decide(self.instructions, self.schema, observation)
        elapsed = time.perf_counter() - started
        answer = result["answers"]["action"]
        if answer["value"] not in self.labels:
        ...
        return PolicyDecision(
            self.labels.index(answer["value"]),
            values,
            float(answer["gamma"]),
            tuple(answer["logits"]),
            answer["provenance"],
            result["usage"],
            elapsed,
            bool(result.get("calibration_fitted", False)),
        )
